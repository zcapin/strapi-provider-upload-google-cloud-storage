...
module.exports = {
  ...
  auth: {
    ...
    baseUrl: {
      ...
      type: 'enum',
      values: ['https://storage.googleapis.com/{bucket-name}', 'https://{bucket-name}', 'http://{bucket-name}'],
    },
  },
  init: config => {
    ...
    return {
      upload: file => {
        return new Promise((resolve, reject) => {
          const backupPath =
            file.related && file.related.length > 0 && file.related[0].ref ? `${file.related[0].ref}` : `${file.hash}`;
          ...
          checkBucket(GCS, config.bucketName, config.bucketLocation)
            ...
            .then(() => {
              if (['.png', '.jpg', '.jpeg', '.tiff', '.bmp', '.gif'].includes(file.ext)) {
            ...
            .then(image => {
              if (!!image) {
                ...
                return Promise.all([
                  new Promise((resolve) => {
                    const imageLG = image.resize(1024, 768);
                    ...
                    imageLG.getBuffer(mimeType, (err, buffer) => {
                      ...
                      GCS.bucket(config.bucketName)
                        ...
                        .then(() => {
                          ...
                          strapi.log.debug(`File successfully uploaded to ${file.urlLG}`);
                          resolve();
                        })
                        .catch(error => {
                          console.warn(error);
                  ...
                  new Promise((resolve) => {
                    const imageMD = image.resize(640, 480);
                  ...
                  new Promise((resolve) => {
                    const imageSM = image.resize(320, 240);
                    ...
                    imageSM.getBuffer(mimeType, (err, buffer) => {
                      ...
                      GCS.bucket(config.bucketName)
                        ...
                        .then(() => {
                          ...
                          strapi.log.debug(`File successfully uploaded to ${file.urlSM}`);
                          resolve();
                        })
                        .catch(error => {
                            resolve();
                  ...
                  new Promise((resolve) => {
                    const imageThumb = image.resize(150, 150);
                    ...
                    imageThumb.getBuffer(mimeType, (err, buffer) => {
                      ...
                      GCS.bucket(config.bucketName)
                        ...
                        .then(() => {
                          ...
                          strapi.log.debug(`File successfully uploaded to ${file.urlThumb}`);
                          resolve();
                        })
                        .catch(error => {
                            resolve();
